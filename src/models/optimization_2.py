...
class ModelOptimizer:
    def __init__(self, X_train, y_train, model_name):
        ...
        # Class Weights 제거 (No Weights)
        # self.scale_pos_weight 제거
    ...
    def objective(self, trial):
        ...
        if self.model_name == 'XGBoost':
            params = {
                ...
                # 'scale_pos_weight': self.scale_pos_weight, # 제거됨
...
def run_optimization():
    ...
    print(f"Loading Dataset for Optimization 2 (No Class Weights): {DATASET_NAME}")
    X_train, y_train, X_test, y_test = load_data(DATASET_NAME)
    
    # LightGBM과 XGBoost 모델만 최적화를 진행합니다.
    models_to_tune = ['XGBoost', 'LightGBM']
    
    for model_name in models_to_tune:
        print(f"\n{'='*40}")
        print(f"Optimizing {model_name} (No Class Weights)...")
        print(f"{'='*40}")
        ...
        if model_name == 'XGBoost':
            ...
            final_model = LGBMClassifier(
                boosting_type='dart', n_estimators=1000, random_state=RANDOM_STATE, verbose=-1, 
        ...
        # 리포트 저장 (이름 변경: _no_cw)
        result_text = (
            f"Model: {model_name} (No Class Weights)\n"
        ...
        save_path = os.path.join(OUTPUT_DIR, f"{model_name.lower()}_no_cw_optimization_report.txt")
